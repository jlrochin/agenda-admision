...
const app = express();
const PORT = process.env.PORT || app.post('/api/citas', upload.single('ordenMedica'), (req, res) => {
    const { numeroExpediente, curp } = req.body;
    const ordenMedica = req.file ? req.file.filename : null;
    ...
    const curpPattern = /^[A-Z]{4}[0-9]{6}[HM][A-Z]{5}[0-9A-Z][0-9]$/; uploadsDir = path.join(__dirname, 'uploads');
    ...
    app.post('/api/citas', upload.single('ordenMedica'), (req, res) => {
        ...
        db.run(
            `INSERT INTO citas_simplificadas (numero_expediente, curp, orden_medica, fecha, hora, estado) 
         VALUES (?, ?, ?, ?, ?, 'pendiente')`,
            [numeroExpediente, curp, ordenMedica, fecha, hora],
            function (err) {
                if (err) {
